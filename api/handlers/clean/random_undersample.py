"""handle_random_undersample handler."""
...
def handle_random_undersample(df: pd.DataFrame, params: dict) -> HandlerResult:
    """Randomly remove majority class rows to balance class distribution."""
    ...
    try:
        ...
        return HandlerResult(success=False, error=f"Random undersampling failed: {e}")
    ...
    return HandlerResult(
        ...
        summary=f"Random undersampling on '{target_col}': {len(df)} → {len(out)} rows. Before: {before}, After: {after}",
    )
